...
public class ViewControl : MonoBehaviour {
    ...
    void TouchMove()
    {
        ...
        if (Input.touchCount == 1) //单点触碰移动摄像机
        {
            ...
            {
                if(DateFile.SecondWorld == 2|| DateFile.SecondWorld == 4)
                {
                    transform.Translate(new Vector3(Input.touches[0].deltaPosition.x * Time.deltaTime * -1, 0, Input.touches[0].deltaPosition.y * Time.deltaTime * -1));
                }else if(DateFile.SecondWorld == 0)
                {
                    transform.Translate(new Vector3(Input.touches[0].deltaPosition.y * Time.deltaTime, 0, Input.touches[0].deltaPosition.x * Time.deltaTime));
                }
                else if (DateFile.SecondWorld == 1)
                {
                    transform.Translate(new Vector3(Input.touches[0].deltaPosition.y * -1 * Time.deltaTime, 0, Input.touches[0].deltaPosition.x * Time.deltaTime * -1));
                }
        ...
        else if (Input.touchCount > 1)//多点触碰
        {
            ...
            for (int i = 0; i < 2; i++)
            {
                ...
                {
                    Camera.main.transform.Translate(isforward * Vector3.down * Time.deltaTime * (Mathf.Abs(deltaDis2.x + deltaDis1.x) + Mathf.Abs(deltaDis1.y + deltaDis2.y)));
    ...
    { 
        if(DateFile.SecondWorld == 1)
        {
            ...
            transform.Translate(new Vector3(y * speed * (-1), mouse * mousespeed * (-1), h * speed * (-1)) * Time.deltaTime, Space.World);
        }
        else if(DateFile.SecondWorld == 0)
        {
            float h = Input.GetAxis("Horizontal");//水平按键
            float y = Input.GetAxis("Vertical");//垂直方向上的按键
            float mouse = Input.GetAxis("Mouse ScrollWheel");//鼠标滚轮
            transform.Translate(new Vector3(y * speed, mouse * mousespeed, h * speed) * Time.deltaTime, Space.World);
